...
import {
  ...
  productDocumentSourceLinks,
  productDocumentEvents,
  productDocumentSnapshots,
  productReports,
  productReportRefreshes,
  productEventWorkspaces,
  productEventWorkspaceEntities,
  productEventWorkspaceEvidence,
  productEventWorkspaceClaims,
  productEventWorkspaceFollowUps,
  productEventBudgetDecisions,
  productEventCaptures,
...
const pipelineRuns = defineTable({
  ...
  ownerKey: v.optional(v.string()), // user:<id>, session:<sessionId>, or trusted service owner
  createdAt: v.number(),
  startedAt: v.optional(v.number()),
  ...
  runId: v.string(),
  // Logical launch/occurrence key. Optional keeps pre-migration rows valid.
  attemptKey: v.optional(v.string()),
  // One durable-workflow instance owns a logical attempt. Retries reuse this
  // key; overlapping workflows for the same scheduled occurrence do not.
  workflowExecutionKey: v.optional(v.string()),
  // Incremented when the owning workflow retries a terminal attempt. Every
  // child write is fenced to this generation so stale executions cannot land.
  executionGeneration: v.optional(v.number()),
  // Idempotency key — hash(pipelineKind + spec + ownerKey + attemptKey)
...
const scheduledPipelineRuns = defineTable({
  // New schedules are authenticated user-owned. Optional preserves legacy rows,
  // which the scheduler now ignores unless the key starts with `user:`.
...
/* Durable, authenticated-owner launch admission. One compact row per user. */
const pipelineLaunchAdmissions = defineTable({
  ownerKey: v.string(),
  shortWindowStartedAt: v.number(),
  shortCount: v.number(),
  dayWindowStartedAt: v.number(),
  dayCount: v.number(),
  updatedAt: v.number(),
...
export default defineSchema({
  ...
  /* ------------------------------------------------------------------ */
  /* TRACE AUDIT LOG - Recorded execution trace with explicit provenance */
  /* Part of the TRACE (Tool-Routed Architecture for Controlled         */
  /* Execution) framework. Entries may describe code or model execution. */
  /* ------------------------------------------------------------------ */
  ...
  traceAuditEntries: defineTable({
    ...
    .index("by_user_execution", ["userId", "executionId", "seq"])
