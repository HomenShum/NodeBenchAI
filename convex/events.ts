 ...
 * (ended rooms keep their row); `liveNow` counts open rooms with recent join or
 * chat activity, so stale test rooms fall out without destructive cleanup. When
 * the backend is unreachable the landing HIDES the stat entirely rather than
 * render a fabricated number (the client enforces the hide).
 */
