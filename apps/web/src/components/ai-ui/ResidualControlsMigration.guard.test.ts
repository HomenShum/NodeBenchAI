...
describe("residual control primitive migration", () => {
  it.each([
    "apps/web/src/features/controlPlane/views/LegalPage.tsx",
    "apps/web/src/features/deepSim/views/DecisionMemoView.tsx",
    "apps/web/src/features/research/views/ReportDetailWorkspace.tsx",
    "apps/web/src/features/research/views/ResearchHub.tsx",
    "apps/web/src/features/strategy/views/ExecutionTraceView.tsx",
    "apps/web/src/features/strategy/views/ProductDirectionMemoView.tsx",
  ...
  it("delegates document filter and density keyboard behavior to ToggleGroup", () => {
    const toolbar = read("apps/web/src/features/documents/components/FiltersToolsBar.tsx");
    const dataHook = read("apps/web/src/features/documents/hooks/useDocumentData.ts");
  ...
  it("models execution disclosure as a single-select toggle group", () => {
    const source = read("apps/web/src/features/strategy/views/ExecutionTraceView.tsx");
  ...
  it("models composer modes, research lanes, and lenses as toggle groups", () => {
    const source = read("apps/web/src/features/product/components/ProductIntakeComposer.tsx");
  ...
  it("uses runtime report identity and ToggleGroup in the reports rail", () => {
    const source = read("apps/web/src/features/redesign/surfaces/HomeV2PrototypeSurface.tsx");
    expect(source).toContain("@/components/ai-ui/toggle-group");
