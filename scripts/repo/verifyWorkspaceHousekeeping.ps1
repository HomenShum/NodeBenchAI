...
if (-not [bool]$loop.augmentScope.candidateCountPassed) {
  Add-Message $failures "augmentScope.candidateCountPassed must be true"
}
if (-not [bool]$loop.augmentScope.criticalIgnoreProbesPassed) {
  Add-Message $failures "augmentScope.criticalIgnoreProbesPassed must be true"
}
if ([int]$loop.augmentScope.criticalIgnoreProbeFailures -ne 0) {
...
$verification = [pscustomobject]@{
  ...
  summary = [pscustomobject]@{
    ...
    criticalIgnoreProbesPassed = $loop.augmentScope.criticalIgnoreProbesPassed
