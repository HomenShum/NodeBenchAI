...
export const goalLoopEvidenceFieldNames = [
  ...
  "developmentBacklogSuggestedVerificationDuplicateCommandCount",
...
export function summarizeCommandEvidence(commands, options = {}) {
  ...
  const duplicateCommandNames = Object.entries(commandOccurrenceCounts)
    ...
    .map(([command]) => command)
    .sort();
  const commandExitCodeHistory = Object.fromEntries(
  ...
  const commandResultRows = commandDurations.map((command, index) => {
    ...
    commandOccurrenceIndexes.set(command.command, occurrenceIndex);
    return {
      index,
      command: command.command,
      occurrenceIndex,
...
export function summarizeVisualEvidence(aestheticReviewReport, context = {}) {
  ...
  const latestVisualArtifactPredatesHeadBySeconds =
    latestVisualArtifactPredatesHead && latestArtifact != null
      ? Math.max(0, Number(((currentHeadCommittedAtMs - latestArtifact.modifiedAtMs) / 1000).toFixed(3)))
      : null;
  ...
  const latestAestheticReviewPredatesHead =
    headCommitKnown && aestheticReviewStat ? aestheticReviewStat.mtimeMs + 1000 < currentHeadCommittedAtMs : false;
  const latestAestheticReviewPredatesHeadBySeconds =
...
export function summarizeDevelopmentBacklogEvidence(developmentBacklog) {
  ...
  return {
    ...
    developmentBacklogSuggestedVerificationDuplicateCommandCount: duplicateSuggestedVerificationCommands.length,
