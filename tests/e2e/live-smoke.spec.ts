...
test.describe("live-smoke — Tier B hydrated-DOM verification", () => {
  ...
  test("/workspace/w/:id mounts UniversalWorkspacePage chromelessly", async ({ page }) => {
    ...
    // Assert the standalone workspace mounted on the requested live artifact.
    await expect(page.locator('[data-agent-contact="standalone-workspace-runtime"]')).toBeVisible({
      timeout: 15_000,
    });
    await expect(page.getByRole("heading", { name: /^acme-ai$/i })).toBeVisible();
    ...
    await expect(page.getByText(/Detail pending - report shell active/i)).toBeVisible();
    await expect(page.getByText(/Live artifact/i).first()).toBeVisible();
  });
